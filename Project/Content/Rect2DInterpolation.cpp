...
void Rect2DInterpolation::onCollisionInterpolation(Collider2D* other)
{
	...
	float x3 = other->GetStartPos().x + WALL_POS.x;
	float y3 = other->GetStartPos().y + WALL_POS.y;
	float x4 = other->GetEndPos().x + WALL_POS.x;
	float y4 = other->GetEndPos().y + WALL_POS.y;
	...
	if (other->GetOwner()->GetLayer() == eLayerType::Slope)
	{
		...
		helper::math::_LineAndLineCollision(x1, y1, x2, y2, x3, y3, x4, y4, &inX, &inY);
	...
	else if (other->GetOwner()->GetLayer() == eLayerType::Wall)
	{
		...
		DebugRenderer2D* debugRenderer = renderer->GetDebugRenderer2D();
		...
		else
		{
			...
			else
			{
				...
				//슬로프가있을땐 왼쪽벽무시가능
				if (mbCollisionDir[static_cast<UINT>(Dir::Slop)] == false)
				{
